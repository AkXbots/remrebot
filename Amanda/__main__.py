...
from Amanda.modules import ALL_MODULES
from Amanda.modules.helper_funcs.alternate import typing_action
from Amanda.modules.helper_funcs.chat_status import is_user_admin
from Amanda.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("Amanda.modules." + module_name)
...
@run_async
def Amanda_about_callback(update, context):
    query = update.callback_query
    if query.data == "aboutmanu_":
        ...
        query.message.edit_text(
            text=f"*{dispatcher.bot.first_name} Is the redisigned version of Amanda v1.0 for the best performance.*"
            f"\n\n{dispatcher.bot.first_name}'s source code was written by 𝒯𝒽𝒶𝓇𝓊𝓀 ℛℯ𝓃𝓊𝒿𝒶"
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        Amanda_about_callback, pattern=r"aboutmanu_"
    )
